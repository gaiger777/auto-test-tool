...
// (□로 보임)로 입력창에 삽입된다. beforeinput 취소로는 네이티브 삽입을 못 막으므로, 삽입된 뒤
// 값에서 그런 문자만 제거하고 커서 위치는 보존한다(React 상태도 네이티브 setter+input 재발생으로 반영).
function isBadCode(c: number): boolean {
  return (
...
document.addEventListener(
  ...
  (e) => {
    ...
    try {
      ...
      /* number 등 selection 미지원 입력 무시 */
    }
    // React onChange가 정리된 값을 반영하도록 input 이벤트 재발생(이미 정리돼 재귀 안 함).

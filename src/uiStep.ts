...
export const kindLabel = (k: UiKind | string): string =>
  ({ click: '클릭', input: '입력', hover: '호버', http_call: 'HTTP', wait_event: '이벤트대기', assert: '검증', sleep: '대기', tab_switch: '탭 전환', file_upload: '파일 업로드' } as Record<string, string>)[k] || k
...
export const stepSummary = (a: UiAction): string => {
  ...
  switch (a.kind) {
    ...
    case 'tab_switch': return `→ 탭 ${a.value ?? '0'}`
    case 'file_upload': return a.value ? `파일: ${a.value}` : '⚠ 파일 미지정'
